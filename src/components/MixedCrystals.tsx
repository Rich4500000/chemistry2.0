...
const InstancedAtoms = ({ positions, color, radius }: { positions: THREE.Vector3[], color: string, radius: number }) => {
  ...
  const geometry = useMemo(() => new THREE.SphereGeometry(radius, 12, 12), [radius]);
  const material = useMemo(() => new THREE.MeshStandardMaterial({ color, roughness: 0.4, metalness: 0.2 }), [color]);
...
const InstancedBonds = ({ bonds, color, radius }: { bonds: {start: THREE.Vector3, end: THREE.Vector3}[], color: string, radius: number }) => {
  ...
  const geometry = useMemo(() => new THREE.CylinderGeometry(radius, radius, 1, 6), [radius]);
  ...
  return (
    <instancedMesh ref={meshRef} args={[geometry, material, bonds.length]} />
  );
};
...
const PzOrbitals = ({ positions }: { positions: THREE.Vector3[] }) => {
  ...
  const geometry = useMemo(() => new THREE.SphereGeometry(0.35, 12, 12), []);
  const topMat = useMemo(() => new THREE.MeshStandardMaterial({ color: '#8b5cf6', roughness: 0.2, transparent: true, opacity: 0 }), []);
  const bottomMat = useMemo(() => new THREE.MeshStandardMaterial({ color: '#ec4899', roughness: 0.2, transparent: true, opacity: 0 }), []);
  ...
  useFrame(({ camera }) => {
    ...
    topMat.visible = opacity > 0.01;
    bottomMat.visible = opacity > 0.01;
  });
  ...
  return (
    <group>
      <instancedMesh ref={topRef} args={[geometry, topMat, positions.length]} />
      <instancedMesh ref={bottomRef} args={[geometry, bottomMat, positions.length]} />
    </group>
  );
};
