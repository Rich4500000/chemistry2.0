...
import { molecularGenerators, CrystalGenerator, BondType } from '../utils/crystalGenerators';
...
const UnitCellBox = ({ position, size }: { position: [number, number, number], size: number }) => {
  return (
    <mesh position={position}>
      ...
      <Edges linewidth={4} color="white" transparent opacity={0.5} />
    </mesh>
  );
};
...
export const MolecularCrystals: React.FC = () => {
  const [activeCrystalId, setActiveCrystalId] = useState<string>(molecularGenerators[0].id);
  const [size, setSize] = useState<number>(2);

  const activeCrystal = useMemo(() => molecularGenerators.find(c => c.id === activeCrystalId)!, [activeCrystalId]);
  ...
  return (
    <div className="flex w-full h-full bg-slate-50">
      <div className="w-1/3 p-6 border-r border-slate-200 bg-white overflow-y-auto flex flex-col">
        <h2 className="text-2xl font-bold mb-4 text-slate-800">分子晶体</h2>
        <p className="text-slate-600 mb-6 text-sm">
          分子晶体是由分子通过分子间作用力（如范德华力、氢键）结合而成的晶体。
        </p>
        ...
        <div className="space-y-2 flex-1 overflow-y-auto pr-2">
          {molecularGenerators.map((crystal) => (
            <motion.button
              key={crystal.id}
