    ...
    for (let y = 0; y < size; y++) {
      for (let z = 0; z < size; z++) {
        for (const b of basis) {
          atoms.push({
            pos: [(x + b.pos[0]) * a, (y + b.pos[1]) * a, (z + b.pos[2]) * a],
            element: b.element
          });
        }
        unitCells.push({
  ...
  for (let i = 0; i < atoms.length; i++) {
    for (let j = i + 1; j < atoms.length; j++) {
      ...
      if (Math.abs(dist - bondLength) < tolerance) {
        bonds.push({ start: atoms[i].pos, end: atoms[j].pos, type: bondType });
      }
    }
  }
...
export const generateHexagonalLattice = (
  ...
  for (let x = 0; x < size; x++) {
    for (let y = 0; y < size; y++) {
      for (let z = 0; z < size; z++) {
        for (const b of basis) {
          const px = (x + b.pos[0]) * a - (y + b.pos[1]) * a / 2;
          const py = (y + b.pos[1]) * a * Math.sqrt(3) / 2;
          const pz = (z + b.pos[2]) * c;
...
export const generateMolecularFCC = (centerElem: string, outerElem: string, a: number, size: number, offset: number) => {
  ...
  const lattice = generateLattice(basis, a, size, 0, 'vdw'); // No VDW bonds drawn
  const bonds: any[] = [];
...
export const molecularGenerators: CrystalGenerator[] = [
  ...
  { id: 'ch4', name: '固态甲烷 (CH₄)', description: '分子晶体，CH₄分子按面心立方排列。', gen: (size) => generateFCC('C', 4.2, size) },
