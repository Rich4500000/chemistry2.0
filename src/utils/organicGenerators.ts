...
const getTetrahedralPositions = (center: THREE.Vector3, refDir: THREE.Vector3, bondLength: number, count: number): THREE.Vector3[] => {
  ...
  // If we have a reference direction (e.g., an existing C-C bond), we align t1 with the INVERSE of that direction
  // so the new bonds point away from the existing bond.
